...
// 用法:
//   VITE_SUPABASE_URL=... VITE_SHARED_AUTH_EMAIL=... VITE_PRIVATE_SPACE_PATH=... \
//   AVATAR_SHARED_PASSWORD=... node scripts/upload-avatars.mjs <L头像图片> <W头像图片>
// 密码只经环境变量进入内存,不会被写入任何文件。
import { readFile } from 'node:fs/promises';
import { createClient } from '@supabase/supabase-js';

const [lFile, wFile] = process.argv.slice(2);
const { VITE_SUPABASE_URL, VITE_SHARED_AUTH_EMAIL, VITE_PRIVATE_SPACE_PATH, AVATAR_SHARED_PASSWORD } = process.env;
...
for (const [name, value] of [['VITE_SUPABASE_URL', VITE_SUPABASE_URL], ['VITE_SHARED_AUTH_EMAIL', VITE_SHARED_AUTH_EMAIL], ['VITE_PRIVATE_SPACE_PATH', VITE_PRIVATE_SPACE_PATH], ['AVATAR_SHARED_PASSWORD', AVATAR_SHARED_PASSWORD]]) {
  if (!value) fail(`缺少环境变量 ${name}(可用 .env.local 或命令行传入)`);
}

const extension = (file) => (file.toLowerCase().endsWith('.jpg') || file.toLowerCase().endsWith('.jpeg') ? 'jpeg' : 'png');

const supabase = createClient(VITE_SUPABASE_URL, anonKey(), { auth: { persistSession: false } });
function anonKey() {
  const key = process.env.VITE_SUPABASE_ANON_KEY;
  if (!key) fail('缺少环境变量 VITE_SUPABASE_ANON_KEY');
  return key;
}
...
for (const [role, file] of [['l', lFile], ['w', wFile]]) {
  ...
  const target = `${VITE_PRIVATE_SPACE_PATH}/avatars/${role}.png`;
